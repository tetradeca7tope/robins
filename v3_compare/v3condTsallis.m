...
functional = 'tsallisDiv';
functionalName = 'Conditional Tsallis-0.75 Divergence';
...
numExperiments = 30; maxN = 4000; numNCands = 10;
nCands = floor(logspace(log10(100), log10(maxN), numNCands));
...
switch distributionIndex
  ...
  case 2
    ...
    trueVal = 1/(functionalParams.alpha-1) * ( mean(renyiDFunc(t)) -1 );
  ...
  for nIter = 1:numNCands
    ...
    for methIter = 1:numMethods
      ...
      switch estimMethods{methIter}
        ...
        case 'DS'
          estim = estimateTwoDistroFunctionals([X,Z],[Y,Z], functional, ...
            functionalParams, paramsds);

        case 'LOO'
          estim = estimateTwoDistroFunctionals([X,Z],[Y,Z], functional, ...
            functionalParams, paramsloo);
