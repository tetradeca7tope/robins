...
%   file_name = sprintf('sel_images/%s.mat', categories{i});
  ...
  num_vecs_in_categ = 0;
  for j = 1:numel(images) %images is the name of the struct
    all_vectors = [all_vectors; images{j}];
    num_vecs_in_categ = num_vecs_in_categ + size(images{j}, 1);
  end
  fprintf('%d features, num_vectors=%d\n', num_vecs_in_categ, size(all_vectors, 2));
end
total_num_images = size(all_vectors, 1);
...
%   shuffled_images = randperm(num_curr_images);
  shuffled_images = 1:num_curr_images;
